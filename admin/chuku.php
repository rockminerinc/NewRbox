...
<?
	...
	$chuku_time =strtotime(trim($_POST['regdate']));//time(); //date('Y-m-d');
	//echo time();
	...
	if(strcmp($chuku_time, "")){	
 
			$DB->db_query("INSERT INTO kucun_sellout (sdate,pid,snum,memo) VALUES ('$chuku_time','$pro_id','$pro_num','$memo')");
			//$oid = $DB->db_insertID();
			echo "<BR><H1>OK!</H1><BR>";
...
<form action="chuku.php" method="post" >
<table border=0>

 
  <tr> <td>出库时间: </td><td> <input class="input" type=text name="regdate" size=12 onclick="fPopCalendar(regdate,regdate);return false" type="text"  value = "<?php echo date("Y-m-d",time());?>"></td></tr>
